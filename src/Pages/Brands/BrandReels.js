...
export default function BrandReels() {
        ...
        try {
            ...
            if (brandReelsResponse && brandReelsResponse.success) {
                ...
                const processedBrandReels = data.map(reel => {
                    // Convert status number to text if needed (for backward compatibility)
                    let statusText = reel.status;
                    if (typeof reel.status === 'number') {
                        if (reel.status === 1) statusText = 'approved';
                        else if (reel.status === 2) statusText = 'rejected';
                    ...
                    return {
                        ...
                        brand_name: reel.brand_name || 'Unknown Brand',
    ...
    const handleDelete = async (id, reelType) => {
        if (window.confirm(`Are you sure you want to delete this reel?`)) {
            try {
                let response;
                if (String(reelType).toLowerCase() === 'brand') {
                    response = await ApiService.deleteBrandReel(id);
                } else {
                    response = await ApiService.deleteReel(id);
                }
                if (response && response.success) {
                    ...
                    const msg = response?.message || 'Failed to delete reel';
                    toast.error(msg);
                }
            } catch (error) {
                ...
                <div className="card">
                    <div className="card-header d-flex align-items-center justify-content-between">
                        ...
                        <div>
                            ...
                            >
                                <i className="ri-add-line align-middle me-1"></i> Add Brand Reel
                            </button>
                        </div>
                    </div>
                                        ...
                                        <tbody>
                                            {currentReels.length > 0 ? (
                                                currentReels.map((reel) => (
                                                    <tr key={reel.id}>
                                                        <td style={{ maxWidth: '300px', whiteSpace: 'normal' }}>{reel.title}</td>
                                                        <td style={{ maxWidth: '200px', whiteSpace: 'normal' }}>{reel.brand_name}</td>
