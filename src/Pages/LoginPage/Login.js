    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                const isInactive = msg.includes('inactive') || msg.includes('not active') || msg.includes('deactive') || msg.includes('disabled');
                setErrors({ general: isInactive ? 'You are inactive' : (response.data.message || 'Login failed') });
            }
        } catch (error) {
            console.error('Login error:', error);
            const emsg = String(error.response?.data?.message || error.message || '').toLowerCase();
            const isInactive = emsg.includes('inactive') || emsg.includes('not active') || emsg.includes('deactive') || emsg.includes('disabled') || error.response?.status === 403;
            setErrors({ general: isInactive ? 'You are inactive' : (error.response?.data?.message || 'Login failed. Please try again.') });
        } finally {
            setLoading(false);
        }
    };
