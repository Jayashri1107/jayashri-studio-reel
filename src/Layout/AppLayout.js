    ...
    useEffect(() => {
        ...
        const adjustLayout = () => {
            ...
            if (window.innerWidth <= 1140) {
                ...
                // Remove sidebar-initial marker if present
                html.classList.remove('sidebar-initial');
        ...
        // Call adjustLayout immediately to ensure correct initial state
        adjustLayout();
        ...
        const handleThemeToggle = (e) => {
            ...
            // Save to localStorage with a stable key so theme persists on refresh
            try {
                const config = JSON.parse(localStorage.getItem('_THEME_CONFIG_') || '{}');
                config.theme = newTheme;
                localStorage.setItem('_THEME_CONFIG_', JSON.stringify(config));
            } catch (e) {
                localStorage.setItem('_THEME_CONFIG_', JSON.stringify({ theme: newTheme }));
            }
        ...
        // Use event delegation on document to avoid DOM manipulation
