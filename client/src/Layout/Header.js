...
export default function Header() {
        ...
        searchTimeoutRef.current = setTimeout(async () => {
            try {
                ...
                const brandReelMatches = brandReelsCache
                    .filter(r => ((r.title || '').toLowerCase().includes(q)) || ((r.category_name || '').toLowerCase().includes(q)))
                    .slice(0, 5)
                    .map(r => ({ label: r.title || 'Untitled', type: 'reel', meta: r.category_name || 'Brand Reel', to: `/brands/reels/view/${r.id}` }));
                ...
                const all = [...brandReelMatches, ...inflReelMatches, ...catMatches, ...productMatches].slice(0, 12);
                setSuggestions(all);
            } finally {
    ...
    }, [query, categoriesCache, brandReelsCache, influencerReelsCache, productsCache]);
    ...
    const userImage = user?.profile?.profileImageUrl || "/assets/images/users/avatar-1.jpg";
